...
def fetch_historical_data(stock_name, years=2):
    ...
    start_date = date.today() - timedelta(days=years*365 + 5)
...
def train_and_predict(stock_name, model_name, prediction_date_str):
    ...
    try:
        ...
        predicted_price_main = model.predict(np.array([[prediction_day_num]]))[0]
        ...
        historical_30_data = {
            ...
            'y': historical_30_days['Close'].tolist(),
        }
        ...
        predicted_future_prices = model.predict(np.array(future_day_nums).reshape(-1, 1)).tolist()
